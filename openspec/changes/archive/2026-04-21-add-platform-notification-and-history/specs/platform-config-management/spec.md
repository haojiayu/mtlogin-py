...
The system SHALL provide a platform configuration list that administrators can view from the management console, and the list SHALL contain a built-in `mt` platform entry.

#### Scenario: Built-in mt platform is available
- **WHEN** an administrator opens the platform configuration view after the upgrade
